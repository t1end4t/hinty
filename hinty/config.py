...
def ensure_config_exists(config_dir: Path, config_path: Path, example_path: Path) -> None:
    """Create config from example if missing, log error and exit."""
    if config_path.exists():
        return
    config_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy(example_path, config_path)
    logger.error(f"Config file created at {config_path}. Please edit it with your values.")
    raise SystemExit(1)
