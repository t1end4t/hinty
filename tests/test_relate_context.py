...
def get_related_files(
    target_file: Path, project_root: Path, all_files: List[Path]
) -> Dict[str, List[Path]]:
    ...
    imports_from = [
        f
        for mod in imports_from_modules
        if (f := resolve_module_to_file(mod, project_root))
    ]
